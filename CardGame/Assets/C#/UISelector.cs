...
public class TwoPlayerUISelector : MonoBehaviour
{
    ...
    public AudioSource audioSource; // 共通で再生するAudioSource
    ...
    void HandleKeyboardInput(int playerIndex, KeyCode up, KeyCode down, KeyCode left, KeyCode right, KeyCode submitKey)
    {
        ...
        if (Input.GetKeyDown(submitKey))
        {
            ...
            // まずDeckSelectButtonがあるか確認
            DeckSelectButton deckButton = uiElements[currentIndices[playerIndex]].GetComponent<DeckSelectButton>();
            if (deckButton != null)
            {
                deckButton.OnClick(playerIndex + 1); // デッキ選択処理
            }
            else
            {
                // DeckSelectButtonがなければ通常のUIボタンを実行
                Button button = uiElements[currentIndices[playerIndex]] as Button;
                if (button != null)
                {
                    button.onClick.Invoke(); // これでDeckSelectUI.OnClick()が呼ばれる
                }
            }
